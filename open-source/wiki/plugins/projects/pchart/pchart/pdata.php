   ...
   function ImportFromCSV($FileName,$Delimiter=",",$DataColumns=-1,$HasHeader=FALSE,$DataName=-1) {
		...
		foreach ($lines as $line) {
			...
			if ( $HasHeader == TRUE && $HeaderParsed == FALSE ) {
				if ( $DataColumns == -1 ) {
					...
					foreach($DataColumns as $column) {
						...
						$this->SetSerieName($Value,"Serie".$Value);
					}
				}
				$HeaderParsed = TRUE;
				continue;
			...
			if ( $DataColumns == -1 ) {
				...
				foreach($Values as $Value) {
					$Value = trim($Value);
					$this->AddPoint($Value,"Serie".$ID);
					$ID++;
				}
			}
			else {
				$SerieName = "";
				if ( $DataName != -1 ) $SerieName = $Values[$DataName];
				foreach($DataColumns as $column) {
					$Value = trim($Values[$column]);
					$this->AddPoint($Value,"Serie".$column,$SerieName);
				}
			}
		}
   }
