...
/*
  Fails
*/
function _test_nextAttr_NULL_ACTUAL() {

    $this->parser->XML_PullParser_getToken();
    $servers=$this->parser->XML_PullParser_getElement('domain');
    $attr_array = $this->parser->XML_PullParser_setAttrLoop();

    $this->assertEquals(NULL, $attr_array,
       "\n\nFails Because without Namespace Support this gets NULL from XML_PullParser_setAttrLoop.\n"
       . "But with Namespace support there's always at least one attribute when the element is assigned to a namespace.\n"
        . "Not clear whether to add a method to test for null attribute arrays.\n"
         . "This would check to see if only attribute in attribute array is '_ns_'\n\n" );
}
